...
namespace GuerillaTrader.Tests
{
    public abstract class GuerillaTraderTestBase : AbpIntegratedTestBase<GuerillaTraderTestModule>
    {
        ...
        protected void UsingDbContext(Action<GuerillaTraderDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GuerillaTraderDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GuerillaTraderDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GuerillaTraderDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GuerillaTraderDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GuerillaTraderDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GuerillaTraderDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GuerillaTraderDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GuerillaTraderDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GuerillaTraderDbContext, Task<T>> func)
        {
            T result;
